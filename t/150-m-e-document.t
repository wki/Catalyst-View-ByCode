...
is($d->as_string, '', 'content is empty');
...
like($d->as_string, qr{<div\s+id="42">\s*</div>}xms, 'content is <div>');
is($d->current_tag->tag, 'div', 'current tag is <div>');

$d->add_text('huhu');
like($d->as_string, qr{<div\s+id="42">\s*huhu\s*</div>}xms, 'content is <div id>huhu</div>');

$d->current_tag->set_attr(id => 97);

$d->set_attr(class => 'fghi');
like($d->as_string, qr{<div\s+class="fghi"\s+id="97">\s*huhu\s*</div>}xms, 'content is <div class id>huhu</div>');
